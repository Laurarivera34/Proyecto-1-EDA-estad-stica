...
ggsave("results/figures/grafico1_mpios_menor_tasa_movimiento.png", 
...
# Guardando el gráfico
ggsave("results/figures/grafico2_mpios_mayor_intencion_mobilidad.png", 
...
ggsave("results/figures/grafico3_razones_migracion.png",
...
# - si demo3 indica que la persona encuestada es jefe/a, usamos demo2
# - si no, usamos demo4 (sexo reportado del/la jefe/a)
...
# Estados civiles considerados para single parent
single_status <- c("Soltero/a", "Separado/a", "Divorciado/a", "Viudo/a")

# Agregado por hogar desde la base de Personas
per_hh <- personas %>%
  group_by(id_hogar) %>%
  summarise(
...
# Resumen de cartagena vs el resto vs total
resumen <- bind_rows(
  hog_single %>%
    mutate(region = if_else(mpio == "Cartagena De Indias", "Cartagena", "Resto país")) %>%
    group_by(region) %>%
...
####3) Presencia de personas con discapacidad

```{r}
# Identificando hogares con alguna persona con discapacidad
...
# 3) Unir con Hogares para región y calcular resúmenes
...
print(resumen_disability)
```

####4) Antecedentes de violencia o amenazas:

```{r}
# Identificando hogares en los cuales hubo violencia para alguno de sus miembros
...
# Resumen Cartagena vs Resto país
...
print(resumen_violencia_amenazas)
```
...
# Distribución del score 0–4 por región (Cartagena / Resto / Total)
...
# 2b) Tabla "ancha": porcentajes por columnas 0,1,2,3,4
...
### a) Estructura de la variable dependiente: Servicios
...
ggsave("results/figures/grafico4_carencias_servicios_cartagena.png",
...
### b) Estructura de las variables independientes:
...
ggsave("results/figures/grafico5_distribucion_jefatura_hogar.png",
...
ggsave("results/figures/grafico7_resutlados_hogar_monoparental.png",
...
print(grafico9)
ggsave("results/figures/grafico8_resultados_discapcidad.png", plot = grafico9, width = 10, height = 6, dpi = 300)
