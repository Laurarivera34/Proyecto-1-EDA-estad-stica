...
-   Estandarizar nombres (mayúsculas/acentos): en este paso ajustamos los nombres de departamento y municipio para mejorar la presentación de los datos de aquí en adelante. Y también se hace un proceso de estandarización usando clean:names() de janitor.
...
color_resalte <- "#016B61" 
color_general <- "#9ECFD4" 
...
# 1. Planteamiento de la pregunta de indagación
...
-   Variable independiente (X): *Score de vulnerabilidad*, construido a partir de tres condiciones clave de vulnerabilidad que se describirán por separado para facilitar su comprensión (jefatura femenina, hogar monoparental, antecedentes de violencia)
...
Para intentar análizar de forma diferencial las diferencias en las carencias de acceso a servicios públicos (variable dependiente), se construyó un **índice de vulnerabilidad** a nivel de hogar que sintetiza tres condiciones estructurales asociadas a un mayor riesgo social. Estas condiciones, consideradas de manera individual y en conjunto, permiten caracterizar la situación de vulnerabilidad de los hogares migrantes en Cartagena de Indias. Las variables consideradas son:
...
3.  **Antecedentes de violencia o amenazas**: registra si algún miembro del hogar ha sido víctima de violencia o amenazas.

A partir de estas cuatro dimensiones se calculó la variable de interés principal, el índice de vulnerabilidad, definido como un puntaje aditivo de 0 a 3 que suma un punto por cada condición cumplida. Este índice ofrece una medida sintética de la vulnerabilidad de los hogares y servirá como variable independiente (X) en los análisis posteriores.

En los siguientes apartados se describe, paso a paso, el procedimiento para identificar cada una de las tres condiciones de vulnerabilidad, con sus respectivos resúmenes para Cartagena, el resto del país y el total nacional, antes de integrar la información en el índice final.
...
3)  **Personas con discapacidad** (esta no se agrega al índice porque solo hay un hogar que tiene persona con discapacidad)
...
Finalmente, el índice de vulnerabilidad es un puntaje aditivo (0–3) que suma tres puntos: (1) jefatura del hogar femenina; (2) hogar mono-parental, definido como jefe/a sin cónyuge y con hijos, o presencia de “Papá/Mamá” con estado civil soltero/separado/divorciado/viudo; (3) antecedente de violencia contra miembros del hogar (proteccion1006_v_ctima_de_violencia_amenazas). El código incluye una comparación de Cartagéna y el país.

```{r}
# 1) Construcción del score de vulnerabilidad (sin discapacidad)
hogares_vulnerabilidad <- hogares_out %>%
  ...
  left_join(hogares_disability %>% select(id_hogar, household_has_disability), by = "id_hogar") %>%  # se mantiene para el análisis univariado
...
grafico_servicios <- ggplot(tabla_resumen, aes(x = servicios_label, y = porc)) +
  geom_col(fill = "#9ECFD4", alpha = 0.9) +
...
grafico6 <- ggplot(tabla_grafico, aes(x = factor(servicios), y = porc, fill = tipo)) +
  ...
  scale_fill_manual(values = c("Sin ponderar" = "#9ECFD4", "Ponderado" = "#016B61")) +
  labs(
...
grafico6_ic <- ggplot(tabla_grafico_ic, aes(x = factor(servicios), y = porc_pond)) +
  geom_col(fill = "#8ABEB9", alpha = 0.9) +
...
save_kable(tabla7_fem, "results/tabla9.html")
...
save_kable(tabla8_fem, "results/tabla10.html")
...
ggsave("results/grafico7.png", plot = grafico_fem, width = 10, height = 6, dpi = 300)
...
# 1.3 Guardar tabla 
save_kable(tabla_resumen_singleparent, "results/tabla11.html")
...
save_kable(tabla7_single, "results/tabla12.html")
...
save_kable(tabla8_single, "results/tabla13.html")
...
save_kable(tabla7_disc, "results/tabla15.html")
...
save_kable(tabla8_disc, "results/tabla16.html")
...
ggsave("results/grafico10.png", plot = grafico_disc, width = 10, height = 6, dpi = 300)
...
# 1.3 Guardar tabla 
save_kable(tabla_resumen_violence, "results/tabla17.html")
...
save_kable(tabla7_viol, "results/tabla18.html")
...
save_kable(tabla8_viol, "results/tabla19.html")
...
ggsave("results/grafico12.png", plot = grafico_viol, width = 10, height = 6, dpi = 300)
...
Explorando en detalle el índice de vulnerabilidad, definida como una variable categórica de 0 a 3, la cual indica la cantidad de vulnerabilidades con las que cuentan los hogares.
...
# 1.3 Guardar tabla
save_kable(tabla_resumen_vul, "results/tabla20.html")
...
# --- 1. Estructura de la variable ---
estructura_vulniv <- data.frame(
  estructura = capture.output(str(cartagena_vul$vulnerability_score))
)

tabla7_vulniv <- estructura_vulniv %>%
  kable("html", caption = "Estructura de la variable: Índice de vulnerabilidad del hogar") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed"))

save_kable(tabla7_vulniv, "results/tabla21.html")


# --- 1.2 Tablas sin y con ponderación ---
# SIN ponderar
tabla_simple_vulniv <- cartagena_vul %>%
  mutate(
    nivel_vul = case_when(
      vulnerability_score == 0 ~ "0: Sin condiciones",
      vulnerability_score == 1 ~ "1: Una condición",
      vulnerability_score == 2 ~ "2: Dos condiciones",
      vulnerability_score == 3 ~ "3: Tres condiciones",
      vulnerability_score == 4 ~ "4: Todas las condiciones",
      TRUE ~ "NA / No sabe"
    )
  ) %>%
  count(nivel_vul) %>%
  ...
  select(nivel_vul, n_sinpond, porc_sinpond)


# CON ponderación (factor de expansión)
tabla_pond_vulniv <- cartagena_vul %>%
  mutate(
    nivel_vul = case_when(
      vulnerability_score == 0 ~ "0: Sin condiciones",
      vulnerability_score == 1 ~ "1: Una condición",
...
# --- 1.3 Unir tablas lado a lado ---
tabla_comp_vulniv <- tabla_simple_vulniv %>%
  full_join(tabla_pond_vulniv, by = "nivel_vul") %>%
  arrange(nivel_vul)

# --- 1.4 Crear tabla HTML ---
tabla8_vulniv <- tabla_comp_vulniv %>%
  kable(
    "html",
    caption = "<b style='color:#003366; font-size:16px;'>Índice de vulnerabilidad del hogar (0 a 4)</b>",
    col.names = c("Nivel de vulnerabilidad", "N (sin pond.)", "% (sin pond.)", "N (pond.)", "% (pond.)"),
    digits = 1
  ) %>%
  add_header_above(c(" " = 1, "Sin ponderar" = 2, "Ponderado" = 2)) %>%
  kable_styling(
...
save_kable(tabla8_vulniv, "results/tabla22.html")


# --- 2. Gráfico comparativo sin y con ponderación ---
tabla_grafico_vulniv <- tabla_comp_vulniv %>%
  select(nivel_vul, porc_sinpond, porc_pond) %>%
  pivot_longer(cols = c(porc_sinpond, porc_pond),
               names_to = "tipo", values_to = "porc") %>%
  mutate(
    tipo = ifelse(tipo == "porc_pond", "Ponderado", "Sin ponderar"),
    nivel_vul = factor(nivel_vul, levels = c(
...
#### g) Visualizaciones exploratorias

Empezamos tratando las variable Y como continua y la variable X como categórica para la exploración inicial.
...
grafico_pond2 <- ggplot(tabla_medias_pond2, aes(x = as.factor(vulnerability_score), y = servicios)) +
  geom_col(fill = "#9ECFD4", alpha = 0.8) +
...
g_comp <- ggplot(res, aes(x = level, y = mean)) +
  geom_col(fill = "#9ECFD4", alpha = 0.85) +
  geom_errorbar(aes(ymin = ci_l, ymax = ci_u), width = 0.15, color = "black") +
...
g_all <- ggplot(res2, aes(y = var_label, x = prop)) +
  geom_col(fill = "#9ECFD4", alpha = 0.9, width = 0.7) +
...
g_vul <- ggplot(df_vul, aes(x = score_f, y = prop)) +
  geom_col(fill = "#9ECFD4", alpha = 0.9, width = 0.6) + 
...
# 2. Evidencia empírica y estimación

Evaluación de los supuestos y la presencia de ties
...
```{r}
# CÁLCULO DE FRECUENCIAS PONDERADAS Y EXPORTACIÓN - INDICE DE VULNERABILIDAD (X)
...
# CÁLCULO DE FRECUENCIAS PONDERADAS Y EXPORTACIÓN - NÚMERO DE CARENCIA (Y)
...
# 4. Exportar la tabla
output_dir <- "results"
file_carencias <- file.path(output_dir, "tabla_carencias_ponderada.html")
save_kable(tabla_carencias, file_carencias)

```

```{r}
# ESTRUCTURA DE LA RELACIÓN (SUPUESTO B)
...
```{r}
# CÁLCULO DE RHO DE SPEARMAN PONDERADO (SUPUESTOS A, C y D)
...
# 3. Interpretación y comunicación de los resultados
...
# Mapa de calor ponderado 
df_heat <- cartagena %>%
  filter(is.finite(vulnerability_score), is.finite(servicios), is.finite(f_exp_hog), f_exp_hog > 0) %>%
  count(vulnerability_score, servicios, wt = f_exp_hog, name = "wt_n") %>%
  group_by() %>%
  mutate(prop = wt_n / sum(wt_n))

grafico_calor <- ggplot(df_heat, aes(x = vulnerability_score, y = servicios, fill = prop)) +
  geom_tile(color = "white") +
  scale_fill_gradient(
    low = "#E6F4F2",   # tono muy claro
    high = "#016B61",  # tono principal
    name = "Masa ponderada",
    labels = scales::percent_format(accuracy = 0.1)
  ) +
  labs(
    title = "Vulnerabilidad vs. Carencias (Mapa de calor ponderado)",
    x = "Índice de vulnerabilidad",
    y = "Número de carencias"
  ) +
  theme_minimal(base_size = 12)
...
# exportando el gráfico
ggsave(
  filename = "results/grafico17.png",
  plot = grafico_calor,
  width = 10,
  height = 7,
  dpi = 300,
...
df_profile <- cartagena %>%
  filter(is.finite(vulnerability_score), is.finite(servicios), is.finite(f_exp_hog), f_exp_hog > 0) %>%
  group_by(vulnerability_score) %>%
  summarise(
    y_mean_w = sum(servicios * f_exp_hog) / sum(f_exp_hog),
    n_w      = sum(f_exp_hog),
    .groups = "drop"
  )

g_tendencia <- ggplot(df_profile, aes(x = vulnerability_score, y = y_mean_w)) +
  geom_line() +
  geom_point(aes(size = n_w), alpha = 0.75) +
  scale_size_continuous(name = "Peso total (N ponderado)", range = c(1.5, 6)) +
  labs(
    title = "Perfil ponderado: media de carencias por nivel de vulnerabilidad",
    x = "Índice de vulnerabilidad",
    y = "Media ponderada de carencias"
  ) +
  theme_minimal(base_size = 12) +
...
ggsave(
  filename = "results/grafico18.png",
  plot = g_tendencia,
  width = 10,
  height = 7,
  dpi = 300,
...
```{r}
# Distribucion de la correlación y el intervalo de confianza 
boots <- boot_res$boots
ci_lo <- as.numeric(quantile(boots, 0.025, na.rm = TRUE))
ci_hi <- as.numeric(quantile(boots, 0.975, na.rm = TRUE))

df_boot <- tibble(rho_boot = boots)

g_boot <- ggplot(df_boot, aes(x = rho_boot)) +
  geom_histogram(
    bins = 60,
    fill = "#9ECFD4",     
    color = "white",      
  ...
  geom_vline(xintercept = rho_obs, linewidth = 0.7, color = "black") +  
  geom_vline(xintercept = ci_lo, linetype = "dashed", color = "black") +
  geom_vline(xintercept = ci_hi, linetype = "dashed", color = "black") +
  labs(
    title = "Distribución bootstrap de ρ (Spearman ponderado)",
    subtitle = paste0(
      "ρ obs = ", round(rho_obs, 4),
      "   |   IC 95% ≈ [", round(ci_lo, 4), "; ", round(ci_hi, 4), "]"
    ),
    x = "ρ* (bootstrap)",
    y = "Frecuencia"
  ) +
...
ggsave(
  filename = "results/grafico19.png",
  plot = g_boot,
  width = 10,
  height = 7,
  dpi = 300,
